...
func (mgr *GroupManager) inviteSetting(update *tgbotapi.Update) {
	if update.Message != nil {
		inviteSetting = services.GetInviteSettings(update.CallbackQuery.Message.Chat.ID)
	}

	inviteSetting.ChatId = update.CallbackQuery.Message.Chat.ID
...
func (mgr *GroupManager) group_invite_status(update *tgbotapi.Update, enable bool) {
	if enable {
...
func (mgr *GroupManager) group_invite_autogenerate(update *tgbotapi.Update, enable bool) {
	if enable {
...
func (mgr *GroupManager) group_invite_notify(update *tgbotapi.Update, enable bool) {
	if enable {
